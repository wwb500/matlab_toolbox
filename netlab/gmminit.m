function mix = gmminit(mix, x, options,kmean_nb_rep)
...
if nargin < 4, kmean_nb_rep = 1; end
...
options(5) = 1;	% get center from data;

centres=cell(1,length(kmean_nb_rep));
optiond=cell(1,length(kmean_nb_rep));
post=cell(1,length(kmean_nb_rep));
errlog=cell(1,length(kmean_nb_rep));

for jj=1:kmean_nb_rep
    [centres{jj}, optiond{jj}, post{jj}, errlog{jj}] = kmeans(mix.centres, x, options);
    errlog{jj}(errlog{jj}==0)=[];
end

finalErrlog=cellfun(@(x) x(end),errlog);
[errlog,indBestRun]=min(finalErrlog);

mix.centres=centres{indBestRun};
optiond=optiond{indBestRun};
post=post{indBestRun};
